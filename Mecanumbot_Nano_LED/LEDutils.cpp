...
void colorWaveFix(CRGB* ledarray, uint16_t numleds, const CRGBPalette16& palette, int direction)
{
  ...
  // Fixed values in animation behavior
  const uint8_t sat8 = 190;                      // Saturation
  const uint8_t brightdepth = 240;               // How low brightness can go
  const uint16_t brightnessthetainc16 = 45 * 256;    // Brightness wave frequency (32 * 256)
  const uint8_t msmultiplier = 50;               // Speed multiplier
  const uint16_t hueinc16 = 800;                 // Hue difference between pixels
  ...
  sHue16 += deltams * 7;  // Also fixed speed here
  ...
  for (uint16_t i = 0; i < numleds; i++) {
    ...
    // Create hue that mirrors between 0–255 and back
...
DEFINE_GRADIENT_PALETTE(green_gp2) {
    0,   1, 14,  5,
  101,  16, 36, 14,
  165,  56, 68, 30,
  242, 150,156, 99,
...
const uint8_t gGradientPaletteCount = sizeof( gGradientPalettes) / sizeof( TProgmemRGBGradientPalettePtr );
  ...
  /*
// =======================================================================================
// ============ Inspiration Below (More palettes if needed)===============================
...
const TProgmemRGBGradientPalettePtr gGradientPalettesInspire[] = {
  Sunset_Real_gp,
  es_rivendell_15_gp,
